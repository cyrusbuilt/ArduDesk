...
#define DEVICE_NAME "ARDUDESK"		                // The device name (hostname).
#define DEVICE_CLASS "ardudesk"                     // The device class (used for discovery).
...
#define CHECK_DESK_HEIGHT_INTERVAL 5000             // How often to chec the sensors.
#define CHECK_MQTT_INTERVAL 60000 * 5               // MQTT connectivity check interval (millis).
#define MQTT_TOPIC_STATUS "ardudesk/status"     	// MQTT status channel to publish to.
...
#endif
const IPAddress defaultIp(192, 168, 0, 229);        // Default static IP address.
...
typedef struct {
    ...
    // Desk height presets
    int16_t sittingHeight;
    int16_t standingHeight;
} config_t;
